...
local function statusBlock(path)
    ...
    local ok, clone = pcall(function() return mod:Clone() end)
    if not ok or not clone then return end
    if mod:IsA("Folder") then
        statusBackup[path] = { clone = clone, isFolder = true, parentPath = path:match("^(.-)%.?[^%.]+$") }
        mod:Destroy()
    elseif mod:IsA("ModuleScript") or mod:IsA("LocalScript") then
        statusBackup[path] = { clone = clone, isFolder = false }
        mod:Destroy()
    end
end
local function statusRestore(path)
    local saved = statusBackup[path]; if not saved then return end
    local existing = statusResolve(path); if existing then pcall(function() existing:Destroy() end) end
    local parentPath = saved.parentPath or path:match("^(.-)%.?[^%.]+$")
    local parent = statusResolve(parentPath)
    if parent then
        pcall(function() saved.clone.Parent = parent end)
    end
...
local function statusToggle(name)
    local g = statusGroups[name]; if not g then return end
    g.on = not g.on
    ...
    if any then pcall(statusTick) elseif statusLoopThread then pcall(task.cancel, statusLoopThread); statusLoopThread = nil end
end
secStatus:Button({ Title = "Toggle: Slowness",       Callback = function() pcall(statusToggle, "Slowness")      end })
secStatus:Button({ Title = "Toggle: Hallucination",  Callback = function() pcall(statusToggle, "Hallucination") end })
secStatus:Button({ Title = "Toggle: Visual Effects", Callback = function() pcall(statusToggle, "Visual")        end })
lp.CharacterAdded:Connect(function()
