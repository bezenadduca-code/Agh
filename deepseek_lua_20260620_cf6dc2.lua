...
local secNosferatu = tabKiller:Section({ Title = "Nosferatu QTE", Opened = true })
...
local keyNameMap = {
    ["E"] = Enum.KeyCode.E, ["R"] = Enum.KeyCode.R, ["F"] = Enum.KeyCode.F,
    ["Q"] = Enum.KeyCode.Q, ["Space"] = Enum.KeyCode.Space,
    ["W"] = Enum.KeyCode.W, ["A"] = Enum.KeyCode.A, ["S"] = Enum.KeyCode.S,
    ["D"] = Enum.KeyCode.D, ["T"] = Enum.KeyCode.T, ["G"] = Enum.KeyCode.G,
    ["H"] = Enum.KeyCode.H, ["1"] = Enum.KeyCode.One, ["2"] = Enum.KeyCode.Two,
...
local function getQTEUI()
    ...
    local tu = pg and pg:FindFirstChild("TemporaryUI")
    return tu and tu:FindFirstChild("QTE")
end

local function getQTEButtonText()
    local q = getQTEUI()
    if not q then return nil end
    for _, name in ipairs({"Button","QTEButton","PromptButton","KeyButton","ActiveButton"}) do
        local b = q:FindFirstChild(name)
        if b and (b:IsA("TextLabel") or b:IsA("TextButton")) and b.Text and b.Text ~= "" then
            return b.Text
        end
    end
    local function search(inst)
        if inst:IsA("TextButton") and inst.Text and inst.Text ~= "" and #inst.Text <= 10 then return inst.Text end
        for _, c in ipairs(inst:GetChildren()) do local r = search(c); if r then return r end end
        return nil
    end
    return search(q)
end

local function handleMobileQTE()
    local q = getQTEUI()
    local b = q and q:FindFirstChild("ActiveButton")
    if b then
        pcall(function() b.MouseButton1Click:Fire() end)
        pcall(function() b.MouseButton1Down:Fire() end)
        pcall(function() b.MouseButton1Up:Fire() end)
        pcall(function() b.Activated:Fire() end)
        return true
    end
    return false
end

local function handlePCQTE()
    local text = getQTEButtonText()
    if not text then return false end
    local key = keyNameMap[text] or keyNameMap[text:sub(1,1):upper()]
    if key then
        pcall(function()
...
local function qteStart()
    ...
    qteThread = task.spawn(function()
        while qte.on do
            local q = getQTEUI()
            if q then
                if not handleMobileQTE() then handlePCQTE() end
            end
            task.wait(qte.speed)
        end
        qteThread = nil
...
secNosferatu:Slider({ Title="Click Speed (s)", Flag="qteSpeed", Step=0.01,
    Value={Min=0.02, Max=0.5, Default=qte.speed},
...
local SHORT_CHARS = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"
local CONFIG_PATH = "WindUI/v1prware/config/v1prware.json"

local function encodeShortConfig(data)
    if not data or not data._elements then return nil end
    local bits = {}
    local function addBits(val, numBits)
        for i = numBits - 1, 0, -1 do
            table.insert(bits, (math.floor(val / 2^i) % 2 == 1) and "1" or "0")
        end
    end
    local keys = {
        "spoofActive", "chatForceEnabled", "platEnabled", "stamOn", "stamNoLoss",
        "speedOn", "hbOn", "acOn", "flowOn", "aimOn", "absOn",
        "sixerStrafeOn", "coolkidWSOOn", "noliVoidRushOn", "qteOn",
        "espEnabled", "espKillers", "espSurvivors", "espGenerators", "espItems", "espStructures",
        "espPizza", "espZombie", "espPuddle", "musicOn",
    }
    local sliderKeys = {
        "stamLoss", "stamGain", "stamMax", "stamCurrent", "speedValue", "hbStrength",
        "acStrength", "acMaxDist", "flowNodeDelay", "flowLineDelay", "aimCooldown",
        "aimLockTime", "aimMaxDist", "aimSmooth", "absRange", "absDur", "qteSpeed",
        "espMaxDistance", "espTransparency", "espMinionTrans",
    }
    local stringKeys = {"timerSide", "platDevice", "musicSel"}
    for _, key in ipairs(keys) do
        local el = data._elements[key]
        if el then
            local val = el.value
            if type(val) == "boolean" then addBits(val and 1 or 0, 1)
            else addBits(0, 1) end
        else addBits(0, 1) end
    end
    for _, key in ipairs(sliderKeys) do
        local el = data._elements[key]
        if el then
            local val = el.value or 0
            if val <= 255 then addBits(val, 8)
            else addBits(255, 8) end
        else addBits(0, 8) end
    end
    for _, key in ipairs(stringKeys) do
        local el = data._elements[key]
        if el then
            local val = tostring(el.value or "")
            for i = 1, #val do addBits(string.byte(val:sub(i,i)), 8) end
            addBits(0, 8)
        else addBits(0, 8) end
    end
    while #bits % 4 ~= 0 do table.insert(bits, "0") end
    local result = "V1PRW:"
    for i = 1, #bits, 4 do
        local val = 0
        for j = 0, 3 do
            val = val * 2 + (bits[i+j] == "1" and 1 or 0)
        end
        result = result .. SHORT_CHARS:sub(val + 1, val + 1)
    end
    return result
end

local function decodeShortConfig(code)
    if not code or not code:match("^V1PRW:") then return nil end
    local chars = code:sub(7)
    local bits = {}
    for i = 1, #chars do
        local val = string.find(SHORT_CHARS, chars:sub(i,i)) - 1
        if val >= 0 then
            for j = 3, 0, -1 do
                table.insert(bits, (math.floor(val / 2^j) % 2 == 1) and "1" or "0")
            end
        end
    end
    local pos = 1
    local function readBits(numBits)
        if pos + numBits - 1 > #bits then return nil end
        local val = 0
        for i = 0, numBits - 1 do
            val = val * 2 + (bits[pos + i] == "1" and 1 or 0)
        end
        pos = pos + numBits
        return val
    end
    ...
    local result = {}
    for _, key in ipairs(keys) do
        local val = readBits(1)
        result[key] = val == 1
    end
    for _, key in ipairs(sliderKeys) do
        local val = readBits(8)
        result[key] = val or 0
    end
    for _, key in ipairs(stringKeys) do
...
secConfigShare:Button({
    ...
    Callback = function()
        local ok,e = pcall(function()
            ...
            print("[v1prware] Full config copied!")
        end)
...
secConfigShare:Button({
    Title = "📋 Copy Short Code (V1PRW:)",
    Icon = "copy",
    Callback = function()
        local ok, err = pcall(function()
            local data = v1prConfig._data
            if not data then print("[v1prware] No config data found"); return end
            local short = encodeShortConfig(data)
            if short then
                setclipboard(short)
                print("[v1prware] Short config copied: "..short)
            end
        end)
        if not ok then print("[v1prware] Copy failed: "..tostring(err)) end
    end
})
...
secConfigShare:Input({
    ...
    Icon = "clipboard",
    Placeholder = "v1prw_full:... or V1PRW:...",
...
secConfigShare:Button({
    ...
    Callback = function()
        ...
        if importStr:match("^V1PRW:") then
            local decoded = decodeShortConfig(importStr)
            if decoded then
                print("[v1prware] Short config decoded!")
                if win and win.Flags then
                    for key, val in pairs(decoded) do win.Flags[key] = val end
                end
                v1prConfig:Load()
                applyConfigFromFlags()
                print("[v1prware] Short config applied!")
            else
                print("[v1prware] Failed to decode short config")
            end
            return
        end
        if importStr:match("^v1prw_full:") then
            ...
            if ok then print("[v1prware] Full config loaded!") else print("[v1prware] Load failed: "..tostring(e)) end
            return
        end
        print("[v1prware] Must start with V1PRW: or v1prw_full:")
    end
})
...
secConfigShare:Button({
    ...
    Icon = "download",
    Callback = function() v1prConfig:Load(); applyConfigFromFlags(); print("[v1prware] Config loaded from file") end
})
