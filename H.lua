...
local function makeDocESP(obj)
    ...
    sparkles.Color = ColorSequence.new{ ColorSequenceKeypoint.new(0, Color3.fromRGB(255,105,180)), ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255,182,213)), ColorSequenceKeypoint.new(1, Color3.fromRGB(255,255,255)) }
    ...
    sparkles.Speed = NumberRange.new(1,3); sparkles.Rate = 20
    sparkles.Lifetime = NumberRange.new(0.8,1.5); sparkles.SpreadAngle = Vector2.new(360,360)
    sparkles.Parent = att
    task.spawn(function()
    ...
    obj.AncestryChanged:Connect(function() if not obj.Parent then foundDoc = false end end)
end

local function docESPOn(on)
    if on then
        foundDoc = false
        for _, v in ipairs(svc.WS:GetDescendants()) do if not foundDoc then makeDocESP(v) end end
        if docESPConn then docESPConn:Disconnect() end
        docESPConn = svc.WS.DescendantAdded:Connect(function(v) if not foundDoc then task.wait(0.1); makeDocESP(v) end end)
    else
        foundDoc = false
        if docESPConn then docESPConn:Disconnect(); docESPConn = nil end
...
secESP:Toggle({ Title="🌸 Document (Jane Doe)", Type="Checkbox", Flag="espDoc", Default=false, Callback=function(on) docESPOn(on) end })
