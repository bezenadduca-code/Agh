...
local _abDotGui     = nil
local _abDotBtn     = nil
local _abPressed    = false   -- true only while finger/mouse is DOWN on the button
local _abDragging   = false
local _abDragStart  = nil
local _abDragPos    = nil
local _abMoveConn   = nil
local _abEndConn    = nil

local function abDotSetState(on)
    if not _abDotBtn then return end
    -- GREEN = AB ON  |  RED = AB OFF
    _abDotBtn.BackgroundColor3 = on
        and Color3.fromRGB(40, 200, 70)
        or  Color3.fromRGB(210, 40, 40)
    _abDotBtn.Text = on and "AB\nON" or "AB\nOFF"
end

local _abToggleRef = nil  -- WindUI toggle element ref, set after creation

local function abDotDoToggle()
    combatS.autoBlockOn = not combatS.autoBlockOn
    abDotSetState(combatS.autoBlockOn)
    -- sync the WindUI checkbox so Settings tab reflects the real state
    pcall(function() if _abToggleRef then _abToggleRef:Set(combatS.autoBlockOn) end end)
...
local function abDotCreate()
    ...
    _abDotBtn.Size                   = UDim2.new(0, 72, 0, 72)
    _abDotBtn.Position               = UDim2.new(1, -88, 0.5, -36)
    _abDotBtn.BackgroundColor3       = Color3.fromRGB(210, 40, 40)
    _abDotBtn.BackgroundTransparency = 0
    _abDotBtn.BorderSizePixel        = 0
    _abDotBtn.Text                   = "AB\nOFF"
    _abDotBtn.TextColor3             = Color3.fromRGB(255, 255, 255)
    _abDotBtn.TextSize               = 15
    ...
    -- ── input: only activate drag/tap tracking when button itself is touched ──
    _abDotBtn.InputBegan:Connect(function(inp)
        if inp.UserInputType ~= Enum.UserInputType.MouseButton1
        and inp.UserInputType ~= Enum.UserInputType.Touch then return end
        _abPressed   = true
        _abDragging  = false
        _abDragStart = inp.Position
        _abDragPos   = _abDotBtn.Position
    end)

    -- move: only runs logic when _abPressed (finger came down on the button)
    _abMoveConn = svc.Input.InputChanged:Connect(function(inp)
        if not _abPressed then return end
        if inp.UserInputType ~= Enum.UserInputType.MouseMovement
        and inp.UserInputType ~= Enum.UserInputType.Touch then return end
        local delta = inp.Position - _abDragStart
        if delta.Magnitude > 10 then _abDragging = true end
        if _abDragging then
            _abDotBtn.Position = UDim2.new(
                _abDragPos.X.Scale, _abDragPos.X.Offset + delta.X,
                _abDragPos.Y.Scale, _abDragPos.Y.Offset + delta.Y
            )
        end
    end)

    -- release: if not a drag → it's a tap → toggle
    _abEndConn = svc.Input.InputEnded:Connect(function(inp)
        if not _abPressed then return end
        if inp.UserInputType ~= Enum.UserInputType.MouseButton1
        and inp.UserInputType ~= Enum.UserInputType.Touch then return end
        local wasDrag = _abDragging
        _abPressed  = false
        _abDragging = false
        if not wasDrag then
            abDotDoToggle()
...
local function abDotDestroy()
    ...
    if _abEndConn  then _abEndConn:Disconnect();  _abEndConn  = nil end
    _abPressed = false; _abDragging = false
    if _abDotGui then
...
_abToggleRef = secInterface:Toggle({ Title = "Auto Block", Type = "Checkbox", Flag = "settingsAutoBlock", Default = combatS.autoBlockOn,
    Callback = function(on)
        combatS.autoBlockOn = on
        if on then
